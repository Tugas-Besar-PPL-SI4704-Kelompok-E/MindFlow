    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-72 bg-white border-r border-gray-200 flex flex-col">
            ...
            <nav class="flex-1 overflow-y-auto py-8">
                ...
                <ul class="space-y-3">
                    <li>
                        <a href="{{ route('admin.dashboard') }}" class="flex items-center px-8 py-4 text-gray-700 hover:bg-purple-50 hover:text-purple-700 font-semibold transition-all">
                    ...
                    <li>
                        <a href="{{ route('admin.rekrutmen') }}" class="flex items-center px-8 py-4 bg-purple-100 text-purple-800 font-bold border-l-4 border-purple-600">
        ...
        <div class="flex-1 flex flex-col overflow-hidden bg-gray-50">
            <header class="h-24 bg-white border-b border-gray-100 flex items-center justify-between px-10">
                <h2 class="text-3xl font-bold text-gray-800">Daftar Rekrutmen</h2>
                <img class="h-12 w-12 rounded-full border-2 border-purple-200" src="https://ui-avatars.com/api/?name=Admin&background=f3e8ff&color=6b21a8">
            </header>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <table class="w-full text-left">
                        <thead class="bg-purple-50 text-purple-900 text-sm font-bold uppercase tracking-widest">
                            <tr>
                                <th class="px-8 py-6">Aplikan</th>
                                <th class="px-8 py-6">Spesialisasi</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-8 py-6">
                                    <div class="text-xl font-bold text-gray-900">{{ $applicant->nama }}</div>
                                    <div class="text-gray-500 text-sm">{{ $applicant->email }}</div>
                                </td>
                                <td class="px-8 py-6 font-medium">{{ $applicant->spesialisasi }}</td>
                                <td class="px-8 py-6">
                                    <span class="px-4 py-1.5 bg-yellow-100 text-yellow-700 rounded-full text-sm font-bold">
                                        {{ $applicant->status }}
                                    </span>
                                </td>
                                <td class="px-8 py-6">
                                    <div class="flex justify-center space-x-4">
                                        <button class="px-6 py-2 bg-purple-600 text-white rounded-xl font-bold hover:bg-purple-700 transition">Verifikasi</button>
                                        <button class="px-6 py-2 bg-red-50 text-red-600 rounded-xl font-bold hover:bg-red-100 transition">Tolak</button>
                                    </div>
                                </td>
                            </tr>
                            @endforeach
