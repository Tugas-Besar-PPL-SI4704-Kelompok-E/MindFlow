            ...
            <div class="flex gap-4 md:gap-5">
                ...
                <div class="absolute top-6 right-6">
                    ...
                    <div id="dropdown-{{ $thread->id }}" class="hidden absolute right-0 top-full mt-1 w-40 bg-white border border-gray-100 rounded-xl shadow-lg py-1.5 z-10">
                        ...
                        @endif
                        @if(Auth::check() && Auth::user()->role !== 'admin' && $thread->user_id !== (Auth::id() ?? 1))
...
<script>
    ...
    document.addEventListener('click', function(e) {
        ...
        const isInsideReport = e.target.closest('[id^="report-post-"]');
        
        if (!isDropdownBtn && !isInsideDropdown && !isInsideReport) {
            document.querySelectorAll('[id^="dropdown-"], [id^="report-post-"]').forEach(d => d.classList.add('hidden'));
        }
    });
