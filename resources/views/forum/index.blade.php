        ...
        <div class="thread-item">
            ...
            @if($thread->user_id === (Auth::id() ?? 1))
                <div class="thread-menu">
                    <form action="{{ route('forum.destroy', $thread->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Hapus post ini?');">
                        @csrf
                        @method('DELETE')
                        <!-- Button as 3 dots -->
                        <button type="submit" style="background:transparent; border:none; cursor:pointer; color:var(--text-muted);" title="Hapus">
